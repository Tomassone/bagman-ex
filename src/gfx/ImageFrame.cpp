...
void ImageFrame::create_like(const ImageFrame &model) {
  ...
  if (m_buffer == 0) {
    abort_run("Cannot create image w=%d, h=%d: %s", model.get_w(),
              model.get_h(), SDL_GetError());
  }
...
ImageFrame::ImageFrame(SDL_Surface *source, bool alpha) { init(source, alpha); }
