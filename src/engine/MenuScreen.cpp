...
GameContext *MenuScreen::private_update(int elapsed_time) {
  ...
  RandomNumber::rand(
      10); // be sure to get a different random at each first game
  ...
  if (m_nb_renders > 0) {
    ...
    m_domain->get_palette().presents.render(m_screen, x_start + 80,
                                            y_start + 40);
    m_domain->get_palette().title.render(m_screen, x_start + 32, y_start + 68);

    y_start += 104;

    m_domain->dark_font.write(m_screen, x_start + 18, y_start,
                              "REMADE BY JOTD 2010-2019");
    m_domain->dark_font.write(m_screen, x_start + 12, y_start + 14,
                              "ORIGINAL BY J.BRISSE 1982");

    // render options

    m_domain->light_font.write(m_screen, x_start + 32, opts_y, "SKILL");
    m_domain->light_font.write(m_screen, x_start + 32, opts_y + 16,
                               "EXTRA LIFE");
  ...
  m_domain->get_palette().barrow.get_first_frame().render(
      m_screen, x_start + 12, opts_y - 8 + m_current_option * 16);

  m_domain->darker_font.write(
      m_screen, opts_x, opts_y,
      MyString(m_options->get_skill_level_str()).uppercase());
  m_domain->darker_font.write(m_screen, opts_x, opts_y + 16,
                              MyString((int)m_options->get_extra_life_score()) +
                                  " PTS");
