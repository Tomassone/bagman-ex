...
#define LOAD_SOUND_12(s)                                                       \
  load_sound(s##_1);                                                           \
  load_sound(s##_2)

#define DECL_SOUND_STRING(s) register_sound_string(SoundSet::s, #s)
#define DECL_SOUND_STRING_12(s)                                                \
  DECL_SOUND_STRING(s##_1);                                                    \
  DECL_SOUND_STRING(s##_2)
...
static std::map<SoundSet::SoundId, int> sound_priority = {
    {SoundSet::player_killed, 20}, {SoundSet::timewarn, 10},
    {SoundSet::reward, 10},        {SoundSet::ethopla, 10},
    {SoundSet::guard_killed, 10},  {SoundSet::hohisse, 10},
    {SoundSet::take_bag, 10},      {SoundSet::pickaxe, 5}};
...
void SoundSet::unload_sound(SoundId k) { m_player.unload(k); }
...
void SoundSet::stop_music() { m_player.stop_music(); }
...
void SoundSet::init(bool silent) { m_silent = silent; }
...
bool SoundSet::is_music_playing() const { return m_player.is_music_playing(); }
...
void SoundSet::load_sound(SoundId k, bool loop) {
  ...
  m_player.load(sound_filename.get_name() +
                    DirectoryBase::get_sound_extension(),
                k, loop, priority);
}

SoundSet::SoundSet()
    : m_player(32000), m_silent(false), m_game_sounds_loaded(false) {}
