...
enum CharacterStartPosSettings {
  CSP_NORMAL,                       // 0
...
void MPLevel::add_character_positions(int px, int py, int pcs,
                                      HumanCharacter::FullDirection pd, int g1x,
                                      int g1y, int g1cs,
                                      HumanCharacter::FullDirection g1d,
                                      int g2x, int g2y, int g2cs,
                                      HumanCharacter::FullDirection g2d) {
...
void MPLevel::handle_guard_bag_collisions() {
  for (int i = 0; i < NB_GUARDS; i++) {
    Guard &g = m_guards[i];
    if (g.get_life_state() == HumanCharacter::ALIVE and
        g.get_state() != HumanCharacter::STATE_IN_AIR) {
      ...
      for (auto ob : items) // GfxObject *
      {
        if (ob->is_moving() && ob->get_bounds().intersects(gb)) {
          if (g.get_state() == HumanCharacter::STATE_CLIMB) {
            ...
            // I have added an extra check to avoid that the y-1 trick to
            // properly detect slopes kills the guard when chasing player on a
            // plane surface
...

{
  m_state = LEVEL_END;
  m_first_level =
      is_trainer_mode(); // change bag layout if not in "trainer" mode
...
void MPLevel::update_pickaxes() {
  static const int pick_locations_01[] = {
      144 + 16 + 3,    192, GfxObject::PICK, 152 + 16 + 3, 64,
      GfxObject::PICK, -1};
  static const int pick_locations_11[] = {
      48 + 16 + 3, 136, GfxObject::PICK, 176 + 16 + 3, 16, GfxObject::PICK, -1};
  static const int pick_locations_21[] = {80 + 16 + 3, 168, GfxObject::PICK,
                                          -1};
...
void MPLevel::private_init() {
  ...
  add_character_positions(0x29, 0xE0, 0, HumanCharacter::FD_RIGHT, 0xB0, 0x10,
                          0, HumanCharacter::FD_LEFT, 0xD0, 0x10, 2,
                          HumanCharacter::FD_RIGHT); // CSP_NORMAL
  add_character_positions(290, 136, 0, HumanCharacter::FD_RIGHT, 0xB0, 0x10, 0,
                          HumanCharacter::FD_LEFT, 0xD0, 0x10, 2,
                          HumanCharacter::FD_LEFT); // CSP_PICK_BREAKS_WALL
  add_character_positions(162, 144, 0, HumanCharacter::FD_RIGHT, 0xB0, 0x10, 0,
                          HumanCharacter::FD_LEFT, 0xD0, 0x10, 2,
                          HumanCharacter::FD_LEFT); // CSP_BAG_ON_SLOPE
  add_character_positions(362 + 50, 16, 0, HumanCharacter::FD_RIGHT, 0xB0, 0x10,
                          0, HumanCharacter::FD_RIGHT, 0xD0, 0x10, 2,
                          HumanCharacter::FD_LEFT); // CSP_ELEVATORS_SCREEN_2
  add_character_positions(224 * 2 + 150, 16, 0, HumanCharacter::FD_RIGHT, 0xB0,
                          0x10, 0, HumanCharacter::FD_LEFT, 0x50, 201, 2,
                          HumanCharacter::FD_LEFT); // CSP_ELEVATORS_SCREEN_3
  add_character_positions(61, 16, 0, HumanCharacter::FD_RIGHT, 61, 224, 0,
                          HumanCharacter::FD_LEFT, 0x50, 201, 2,
                          HumanCharacter::FD_LEFT); // CSP_BARROW_KILL
  add_character_positions(115, 146, 0, HumanCharacter::FD_RIGHT, 109, 99, 0,
                          HumanCharacter::FD_RIGHT, 0x50, 201, 2,
                          HumanCharacter::FD_LEFT); // CSP_GUARD_SLOPE_TURN
  add_character_positions(
      303, 96, 0, HumanCharacter::FD_RIGHT, 341, 64, 0,
      HumanCharacter::FD_RIGHT, 0x50, 201, 2,
      HumanCharacter::FD_LEFT); // CSP_ELEVATORS_OUT_SCREEN_2
  add_character_positions(
      303, 96, 0, HumanCharacter::FD_RIGHT, 194, 192, 0,
      HumanCharacter::FD_LEFT, 209, 200, 1,
      HumanCharacter::FD_LEFT); // CSP_GUARD_WALK_CLIMB_BRANCH
  add_character_positions(
      32, 176, 0, HumanCharacter::FD_RIGHT, 62, 70, 0, HumanCharacter::FD_UP,
      0xD0, 0x10, 2, HumanCharacter::FD_LEFT); // CSP_GUARD_WALK_CLIMB_BRANCH
  add_character_positions(
      224 * 2 + 150, 16, 0, HumanCharacter::FD_RIGHT, 0xB0, 0x10, 0,
      HumanCharacter::FD_LEFT, 535, 40, 0,
      HumanCharacter::FD_LEFT); // CSP_ELEVATORS_TOP_SCREEN_3
  add_character_positions(224 * 2 + 32, 16, 0, HumanCharacter::FD_RIGHT, 0xB0,
                          0x10, 0, HumanCharacter::FD_LEFT, 0x50, 201, 2,
                          HumanCharacter::FD_LEFT); // CSP_GUARD_ELEVATORS_CHEAT

  m_to_right_input.right.pressed(
      true); // fake input to flee right when all bags have been collected
  ...
  m_elevators[0].init(this, &m_palette->elevator_red, 0x11, 0x89, red_stops,
                      true, false);
  ...
  m_elevators[1].init(this, &m_palette->elevator_orange, 0x11, 0xC9,
                      orange_stops, true, false);
...
void MPLevel::init_new_level() {
  ...
  static const int bag_locations_01[] = {112 + 16,
                                         16,
                                         GfxObject::BARROW,
                                         160 + 16 + 6,
                                         144,
                                         GfxObject::YELLOW_BAG,
                                         120 + 16 + 6,
                                         64,
                                         GfxObject::YELLOW_BAG,
                                         24 + 16 + 6,
                                         ...
                                         GfxObject::YELLOW_BAG,
                                         -1};
  static const int bag_locations_11[] = {112 + 16 + 6,
                                         64,
                                         GfxObject::YELLOW_BAG,
                                         176 + 16 + 6,
                                         80,
                                         ...
                                         GfxObject::BLUE_BAG,
                                         -1};
  static const int bag_locations_21[] = {200 + 16 + 6,
                                         16,
                                         GfxObject::YELLOW_BAG,
                                         136 + 16 + 6,
                                         40,
  ...
  static const int bag_locations_02[] = {112 + 16,
                                         16,
                                         GfxObject::BARROW,
                                         200 + 16 + 6,
                                         136,
  ...
  static const int bag_locations_12[] = {176 + 16 + 6,
                                         80,
                                         GfxObject::YELLOW_BAG,
                                         176 + 16 + 6,
                                         104,
  ...
  static const int bag_locations_22[] = {8 + 16 + 6,
                                         136,
                                         GfxObject::YELLOW_BAG,
                                         96 + 16 + 6,
                                         168,
...
bool MPLevel::is_trainer_mode() const {
  return (m_domain->get_menu_options().get_skill_level() ==
          OptionsBase::TRAINER);
}
...
void MPLevel::stop_music() { m_domain->sound_set.stop_music(); }
void MPLevel::restart() {
  ...
  for (int i = 0; i < NB_GUARDS; i++) {
    m_guards[i].level_init(csps.guard[i].x, csps.guard[i].y,
                           csps.guard[i].current_screen, csps.guard[i].d);
  }

  m_player->level_init(csps.player.x, csps.player.y, csps.player.current_screen,
                       csps.player.d);

  if (CURRENT_CSP == CSP_BARROW_KILL) {
    static const int pick_locations_test[] = {40 + 16 + 3, 16, GfxObject::PICK,
                                              -1};
...
void MPLevel::store_character_current_position(const Renderable *c, int delta_x,
                                               int delta_y) {
  int cs = m_player->get_current_screen();
...
void MPLevel::store_current_positions() {
  ...
  for (auto &p : m_redraw_bounds) {
    if (p.second != nullptr) {
      if (not p.second->is_in_screen(cs)) {
        m_late_redraw_bounds[late_redraw_bounds_index++] =
            std::make_pair(p.first, nullptr);
      } else {
        // bags
  ...
  // this part is not very "object oriented"... I don't want to create virtual
  // methods for x/y margins, plus the fact that there's a special case for the
  // player & guards in order to optimize redraws
...
GameContext *MPLevel::update_running(int elapsed_time) {
...

  {
    ...
    if ((prev_screen != current_screen) || m_first_update) {
      ...
      if ((current_screen == 0 || current_screen == 2) and
          RandomNumber::rand(2) == 0) {

        MyString track_name = DirectoryBase::get_sound_path() / "track" +
                              MyString(current_screen + 1) + ".mp3";
  ...
  case HumanCharacter::DEAD: {
    ...
    // increase to 2500 millis so death sound can end playing before music
    // restarts
...
GameContext *MPLevel::private_update(int elapsed_time) {
  ...
  if (m_state == RUNNING && m_level_end_fadeout &&
      m_fadeout_event.is_timeout_reached()) {
    stop_music();
    m_screen.fill_rect(0, 0);
    m_level_end_fadeout = false;
  }
  if ((m_gameover_fadeout || m_state == QUIT_GAME) &&
      m_fadeout_event.is_timeout_reached()) {
    stop_music();
    m_screen.fill_rect(0, 0);
    // return to menu
...
void MPLevel::pick_up_object(GfxObject *o) {
  ...
  GfxObjectLayer::List &l =
      m_object_layer[m_player->get_current_screen()].get_items();
  unsigned int nb_items = l.size();
  l.remove(o, false);
  if (l.size() != nb_items - 1) {
    abort_run("cannot remove item %p from screen %d. barrow = %p", o,
              m_player->get_current_screen(), get_barrow());
  }
...
#ifdef PLAYER_ALONE
    if ((&c == &m_guards[0]) or (&c == &m_guards[1]))
      continue;
#endif
...
void MPLevel::get_elevator_contained_state(const Character &c,
                                           Elevator::CharacterContained &cc,
                                           const Elevator *&e) const {
...
void MPLevel::render_breakable_block() {
  ...
  if (m_nb_pick_blows < 5) {
    m_palette->wall_break.get_frame(m_nb_pick_blows)
        .render(m_screen, x, 176 + 16);
  }
}
...
// draw background
    ...
    m_screen.set_affine_transformation(SCALE_SIZE, m_domain->rotate_90,
                                       X_TRANSLATION - 224 * cs, y_translation);
