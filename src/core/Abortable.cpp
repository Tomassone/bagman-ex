...
static ThreadSafeContainer<NameSortProfilerTable>
    m_profiler_table_list; ///< custom profiling
...
static EntryPointList &m_entrypoint_list() { return m_stack_map.get(); }
...
#define MSG_ARG_PROTO                                                          \
  const MyString &message, const MyString &arg1, const MyString &arg2,         \
      const MyString &arg3, const MyString &arg4, const MyString &arg5,        \
      const MyString &arg6, const MyString &arg7, const MyString &arg8,        \
...
Abortable::Abortable(const MyString &name, bool user_level_object)
    : m_name(name), m_user_level_object(user_level_object) {}

#ifdef _HAS_THREADS

/* static */ bool Abortable::is_within_context(const char *object_name,
                                               bool by_class) {
  bool rval = false;
  ...
  for (it = stk.begin(); it != stk.end() && !rval; it++) {
    ...
    rval = (strcmp(by_class ? it->m_class.c_str() : it->m_name.c_str(),
                   object_name) == 0);
  }
...
void Abortable::set_name(const MyString &name) { m_name = name; }
bool Abortable::pop_stack_to_current() const { return false; }
#else
const Abortable::EntryPoint &Abortable::get_current_entrypoint() const {
...
MyString Abortable::stack_trace() {
  ...
  if (!stk.empty()) {
    ...
    while (it != stk.end()) {
      ...
      rval += MyString(ep.m_name) + "(" + ep.m_class + ")::" + ep.m_method +
              " (called from: " + MyString(ep.m_filename).basename() +
...
bool Abortable::pop_stack_to_current() const {
  ...
  while (!stk.empty() && (stk.begin()->m_reference != this)) {
    stk.erase(stk.begin());
...
MyString Abortable::get_location_prefix(const EntryPoint &ep) {
  ...
  return (
      ep.m_name + MyString("(") + ep.m_class + MyString(")") +
      ((ep.m_method[0] == '?') ? ": " : MyString("::") + ep.m_method + ": "));
}
...
void Abortable::abort_run(MSG_ARG_PROTO) const {
  ...
  throw(Cause(format_message(message, MSG_ARG_LIST), get_current_entrypoint()));
#else
  ...
  while (idx < msgsize) {
    ...
    if ((current != '%') || (idx == msgsize)) {
      ...
      case '9': {
        if (argindex < argcount) {
          int enddigit = idx;
          while ((enddigit < msgsize - 1) && (message[enddigit] != 's')) {
            enddigit++;
          }
...
MyString Abortable::system_error_reason() { return strerror(errno); }
...
#ifndef _HAS_THREADS
  printf("%s%s", get_location_prefix(get_current_entrypoint()).c_str(),
         msg.c_str());
...
#ifdef _HAS_THREADS
void Abortable::enter_method(const char *mname, const char *fname,
  ...
  stack_push(EntryPoint(this, mname, fname, line_number, debug_level));
...
void Abortable::exit_method(const char *fname, const int line_number) const {
  const int debug_level = get_debug_level();
  EntryPointList::Stack &stk = m_stack();

  if (stk.empty()) {
    abort_run("empty stack at line %d, file %q", line_number, fname);
  }
...
Abortable::ProfilerTable Abortable::get_profiler_info() {
  ...
  for (it = m_profiler_table_list.begin(); it != m_profiler_table_list.end();
       it++) {
    // for each thread
...
Abortable::Cause::Cause(const MyString &error_message,
    ...
    : EntryPoint(entrypoint), m_message(error_message) {
