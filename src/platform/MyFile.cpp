...
bool MyFile::del() { return (remove(m_name.c_str()) == 0); }

StreamPosition MyFile::write_all(const StreamPosition length_to_write,
  ...
  if (f != 0) {
    // size_t oldpos = 0;
    length_written =
        fwrite((const char *)buffer, length_to_write * sizeof(char), 1, f);
...
void *MyFile::read_all(StreamPosition &length_read,
                       const StreamPosition max_size) const {
  ...
  if (exists()) {
    ...
    if (d != NULL) {
      ...
      while ((strdir = readdir(d)) != NULL) {
        ...
        if ((isrep) && (include_subdirs) && (namebase != "..") &&
            (namebase != ".")) {
          // call the function with the name of the subdir
          // the same list, and the append mode

          if (!rep.dir(elts, hide_dirs, hide_files, absolute_path,
