...
MyString::MyString(const char *string_list[], char quote_char_start,
                   char quote_char_end, char separation_char, int item_count)
    : m_contents(0) {
  int counter = 0;
  build("");
  MyString &me = *this;

  while ((string_list[counter] != NULL) &&
         ((item_count < 0) || (counter < item_count))) {
...
MyString::MyString() : m_contents(0) { build("", 0); }
...
MyString::MyString(const unsigned long int val, bool hexadecimal)
    : m_contents(0) {
  ostream_build(val, hexadecimal ? "%ulx" : "%ul");
}

MyString::MyString(const char *str) : m_contents(0) { build(str); }
...
MyString::MyString(const unsigned long long int val, bool hexadecimal)
    : m_contents(0) {
#ifndef _WIN32
  ostream_build(val, hexadecimal);
#else
...
void MyString::build(const char *str) { build(str, strlen(str)); }
...
template <class T> void MyString::ostream_build_float(const T &val, const int) {
  char buf[200];
...
bool MyString::is_absolute_path(bool eval_env_variables) const {
  MyString copy = (eval_env_variables ? this->eval_env_variables() : *this);
...
MyString MyString::left(const char separator, const bool end) const {
  char buf[2];
  buf[0] = separator;
  buf[1] = '\0';
  return (left(buf, end));
...
MyString MyString::left(const char *separator, bool end) const {
  size_type possep;
...
MyString MyString::right(const char separator, const bool end) const {
  char buf[2];
  buf[0] = separator;
  buf[1] = '\0';
...
bool MyString::int_value(unsigned long long &value,
                         const bool hexadecimal) const {
  return p_int_value(value, hexadecimal);
}
#else
...
template <class FLT_TYPE> bool MyString::p_float_value(FLT_TYPE &value) const {
...
MyString::size_type
MyString::find_first_of(const char *chars,
                        const MyString::size_type start_pos) const {
  return find_xxx_of(chars, start_pos, true, false);
}
MyString::size_type
MyString::find_last_of(const char *chars,
                       const MyString::size_type start_pos) const {
  return find_xxx_of(chars, start_pos, false, false);
}
MyString::size_type
MyString::find_first_not_of(const char *chars,
                            const MyString::size_type start_pos) const {
  return find_xxx_of(chars, start_pos, true, true);
}

MyString::size_type
MyString::find_last_not_of(const char *chars,
                           const MyString::size_type start_pos) const {
...
MyString::size_type MyString::find_xxx_of(const char *chars,
  ...
  if (sz > 0) {
    ...
    int nb_loops = (forward ? (sz - start_pos) : start_pos);
...
MyString
MyString::replace(const MyString &name1, //: I : string to look for
...
MyString::size_type
MyString::string_find(const MyString &substr,
                      const MyString::size_type start_pos) const {
  size_type rval = UINT_MAX;

  if (start_pos < size()) {
    const char *occ = strstr(c_str() + start_pos, substr.c_str());
