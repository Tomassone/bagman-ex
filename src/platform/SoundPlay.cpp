...
SoundPlay::~SoundPlay() { close(); }

#define NUM_SOUNDS 16

#ifdef USE_SDL_MIXER

SoundPlay::SoundPlay(int master_sample_rate)
    : audio_open(false), m_master_sample_rate(master_sample_rate) {

  m_music = 0;
}
bool SoundPlay::is_music_playing() const { return Mix_PlayingMusic() != 0; }
...
SoundPlay::SampleNode *SoundPlay::load(const MyString &filepath, int key,
                                       bool loop, int) {
  ...
  if (audio_open) {
    ...
    if (c == NULL) {
      ...
      std::pair<SampleList::iterator, bool> rval =
          sample_node.insert(std::make_pair(key, sn));
...
void SoundPlay::init() {
  ...
  if (!audio_open) {
    if (Mix_OpenAudio(m_master_sample_rate, MIX_DEFAULT_FORMAT, 2, 4096) ==
        -1) {
...
void SoundPlay::mixaudio(Uint8 *stream, int len) {
  ...
  for (i = 0; i < NUM_SOUNDS; ++i) {
    ...
    if (amount > 0) {
      SDL_MixAudio(stream, &s.data[s.dpos], amount, SDL_MIX_MAXVOLUME);
      s.dpos += amount;
...
void SoundPlay::unload_all() {
  ...
  SampleList::iterator it;
  FOREACH(it, sample_node) { unload(it->first); }
  sample_node.clear();
}
