...
SDL_Surface *ImageUtil::load_image(const MyString &image_file) {
  ...
  if (temp_image != 0) {
    ...
    // get rid of any exotic format from the loaded file: convert to the same
    // format
...
SDL_Surface *ImageUtil::replace_color(const SDL_Surface *src, int old_color,
                                      int new_color, bool) {
...
void ImageUtil::rotate_90(const SDL_Surface *input, SDL_Surface *output,
                          bool positive) {
  // sanity checks
  int iw = input->w;
  int ih = input->h;
...
void ImageUtil::replace_color_in_place(SDL_Surface *rval, int old_color,
                                       int new_color) {
...
SDL_Surface *ImageUtil::mirror(const SDL_Surface *source) {
  ...
  SDL_SetColorKey(dest, SDL_RLEACCEL | SDL_SRCCOLORKEY,
...
void ImageUtil::set_transparency(SDL_Surface *image, int rgb) {
  Uint32 colorkey = SDL_MapRGB(image->format, (rgb & 0xFF0000) >> 16,
...
/*
   void ImageUtil::render_half_size(const SDL_Surface *source, SDL_Surface
   *dest)
   {
...
SDL_Surface *ImageUtil::clone_image(const SDL_Surface *image) {
  ...
  SDL_SetColorKey((SDL_Surface *)image, SDL_RLEACCEL | SDL_SRCCOLORKEY,
...
SDL_Surface *ImageUtil::create_alpha_image(int w, int h) {
  ...
  rval = SDL_CreateRGBSurface(SDL_HWSURFACE | SDL_SRCALPHA, w, h, DISPLAY_DEPTH,
...
SDL_Surface *ImageUtil::create_image(int w, int h) {
  ...
  // always create a temporary surface of 8 bits, and convert it with
  // DisplayFormat to avoid all sort of color problems

  SDL_Surface *rval_temp =
      SDL_CreateRGBSurface(SDL_HWSURFACE, w, h, 8, rmask, gmask, bmask, 0);
...
void ImageUtil::set_pixel(SDL_Surface *pSurface, int x, int y, Uint32 rgb) {
  ...
  // copy pixel data: the only memcpy function here: not clean, but the only way
  // to do it quickly I think this works only in little endian architecture
